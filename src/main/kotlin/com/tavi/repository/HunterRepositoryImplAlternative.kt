...
class HunterRepositoryImplAlternative: HunterRepositoryAlternative {

    override val hunters = listOf(
        Hunter(
            id = 1,
            name = "Gon Freecss",
    ...
    override suspend fun getAllHunters(page: Int, limit: Int): ApiResponse {
        return ApiResponse(
            success = true,
            message = "ok",
            prevPage = calculatedPage(
                hunters = hunters,
                page = page,
                limit = limit
            )["prevPage"],
            nextPage = calculatedPage(
                hunters = hunters,
                page = page,
                limit = limit
            )["nextPage"],
            hunters = provideHunters(
                hunters = hunters,
                page = page,
                limit =limit
            ),
    ...
    private fun calculatedPage(
        hunters: List<Hunter>,
        page: Int,
        limit: Int
    ): Map<String, Int?> {
        val allHunters = hunters.windowed(
            size = limit,
            step = limit,
            partialWindows = true
        )
        require(page <= allHunters.size)
        val prevPage = if(page == 1) null else page - 1
        val nextPage = if(page == allHunters.size) null else page + 1
        return mapOf(
    ...
    private fun provideHunters(
        ...
        require(page > 0 && page <= allHunters.size)
        return allHunters[page - 1]
    }
